...
def get_unique_beacons(scanners):
    ...
    for scanner in scanners.values():
        ...
        beacons.extend(scanner_beacons)
...
def get_adjustments(
    ...
    binary_negation = bin(negation)[2:]
...
def find_unknown_scanners(scanners, positions):
    for position in positions:
        ...
        for scanner_number, scanner_data in deepcopy(scanners).items():
            if scanner_data['position'] is None:
                scanners = try_to_find_scanner(scanners, known_scanner_number, scanner_number)
...
def count_beacons(scanners):
    ...
    while len(positions) < len(scanners):
        scanners = find_unknown_scanners(scanners, positions)
        positions = get_positions(scanners)
...
def handler():
    ...
    # answer = get_beacon_differences(scanners)[0]
    # answer = scanners[1]
