...
def find_lowest_risks(risk_dict, start):
    ...
    while unvisited:
        ...
        for connection in risk_dict[current]['connections']:
            ...
            if connection in unvisited and connection not in possibilities:
...
def get_risk_graph(risk_lists):
    risk_dict = {}
    start_x_len = len(risk_lists[0])
    start_y_len = len(risk_lists)
    x_len = start_x_len * 5
    y_len = start_y_len * 5
    ...
    for i in range(y_len):
        for j in range(x_len):
            ...
            if j + 1 < x_len:
                connections.append((i, j + 1))
            steps_removed = i // start_y_len + j // start_x_len
            value = int(risk_lists[i % start_y_len][j % start_x_len]) + steps_removed
            if value > 9:
                value = value - 9
            risk_dict[i, j] = {
